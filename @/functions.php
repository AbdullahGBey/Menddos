...
class user
{
	...
	function hasMembership($odb)
	{
		...
		{
			$SQLupdate = $odb -> prepare("UPDATE `users` SET `rank` = 0, `membership` = 55, `expire` = 2147472000 WHERE `ID` = :id");
	...
	{
		$parameters = array("<script", "alert(", "<iframe", ".css", ".js", "<meta", "<frame", "<embed", "<xml", "<SCRIPT", "<IFRAME", "<META", "<FRAME", "<EMBED", "<img", "window.location", "window.location.hostname", "</p>", ">", "<");
...
class stats
{
	...
	{
		$SQL = $odb -> query("SELECT COUNT(*) FROM `users` WHERE `membership` != 55");
		return $SQL->fetchColumn(0);
	}
