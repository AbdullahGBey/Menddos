...
if ($type == 'all') {
    ...
    if ($stats->runningBoots($odb) > $maxattacks && $maxattacks > 0) {
        die(error('Attacks slots for this method are full, try another method or wait some minutes!'));
    }
...
if ($type == 'start' || $type == 'renew') {
    ...
    if ($system == 'api') {
        //Check rotation
        $i            = 0;
        $SQLSelectAPI = $odb->prepare("SELECT * FROM `api` WHERE `methods` LIKE :method ORDER BY RAND() LIMIT 1");
		$SQLSelectAPI -> execute(array(':method' => "%{$method}%"));
        while ($show = $SQLSelectAPI->fetch(PDO::FETCH_ASSOC)) {
            if ($rotation == 1 && $i > 0) {
                break;
...
';
        ...
        if ($show['time'] + $show['date'] > time() and $show['stopped'] != 1) {
            ...
            $action = '
			<button type="button" id="rere" onclick="(' . $rowID . ')" class="btn btn-xs btn-effect-ripple btn-primary">
																	<span class="btn-ripple animate"></span><i class="fa fa-refresh"></i>  Nothing
																	</button>';
        }
       	   ?>	   
